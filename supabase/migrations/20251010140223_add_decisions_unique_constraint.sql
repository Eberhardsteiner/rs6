...
-- Step 3: Remove duplicate entries (keep the newest by updated_at)
-- Create a temporary table with unique entries
CREATE TEMP TABLE decisions_unique AS
SELECT DISTINCT ON (game_id, player_id, day, block_id) *
FROM decisions
ORDER BY game_id, player_id, day, block_id, updated_at DESC NULLS LAST;

-- Step 4: Delete all rows from original table
DELETE FROM decisions;

-- Step 5: Re-insert unique rows
INSERT INTO decisions
SELECT * FROM decisions_unique;

-- Step 6: Drop temporary table
DROP TABLE decisions_unique;
...
-- Step 8: Add composite index for common query patterns
CREATE INDEX IF NOT EXISTS idx_decisions_game_player_day_block
  ON decisions(game_id, player_id, day, block_id);

-- Step 9: Add trigger to auto-update updated_at on changes
