...
// Realtime: Spieler-Liste für diese Lobby live halten
  ...
  const getRoleCard = (role: RoleId) => {
    // Case-insensitive Vergleich und Live-Daten verwenden
    ...
    // Aktiv, wenn Spiel läuft und der Spieler kürzlich gesehen wurde
    const gameIsRunning = liveGame.state === 'running' || liveGame.status === 'running';
    const isActive =
      !!player &&
      gameIsRunning &&
      !!player.last_seen &&
      (Date.now() - new Date(player.last_seen).getTime()) < 120000;
