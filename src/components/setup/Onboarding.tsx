...
import { infoContents } from '@/data/infoContent'; // Import infoContents for type definition

type Props = { onStart: (name: string, roles: RoleId[]) => void; onShowModal: (type: 'privacy' | 'imprint' | 'disclaimer' | null) => void };
...
/** Kleine, ruhige Sparkline (kann per CSS ausgeblendet werden) */
function Sparkline({
...
export default function Onboarding({ onStart, onShowModal }: Props) {
  // --- State ---
  const [name, setName] = React.useState('');
  const [roles, setRoles] = React.useState<RoleId[]>(['CEO']); // Mehrfachauswahl möglich
  ...
  const handleStart = () => {
    const n = name.trim() || 'Spieler:in';
    const picked = roles.length ? roles : ['CEO'];
    onStart(n, picked);
  };
    ...
    <div className="ob-root">
      ...
      <header className="ob-header">
        <div className="ob-header-inner">
          <img 
            ...
            className="ob-logo"
          />
          <h1 className="ob-header-text">LQ14 – Liquiditätskrise  EINZELSPIELERMODUS</h1>
        </div>
      </header>
      ...
      <div className="ob-grid">
            ...
            <div className="ob-hint-box" style={{ marginTop: 12 }}>
              ...
              <div>
                ...
                <div className="ob-hint-text">
                  ...
                  werden erst beim Tageswechsel auf die KPI angewandt.
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="ob-card ob-roles">
          <header className="ob-card-h">
            <span className="ob-kicker">Rollen</span>
            <h2>Wählen Sie Ihre Verantwortlichkeit(en)</h2>
          </header>

          <div className="ob-role-grid">
            {ROLES.map((r) => {
              const isSel = roles.includes(r.id);
              return (
                <div
                  key={r.id}
                  role="button"
                  tabIndex={0}
                  className={`ob-role ${isSel ? 'sel' : ''}`}
        ...
        <section className="ob-card ob-howto">
          <header className="ob-card-h">
            ...
            <h2>So bedienen Sie die Simulation</h2>
          </header>
          <ul className="ob-howto-list">
            <li>
              <IconCheck /> Pro Tag treffen Sie je Rolle <strong>mindestens vier Entscheidungen</strong>.
              Jede enthält vier Optionen (A–D) plus ein Freitextfeld ohne KPI-Wirkung (für das Protokoll).
            </li>
            <li>
              <IconTimer /> <strong>Timer</strong>: grün → gelb → rot. Nach Ablauf + <strong>3 Minuten</strong> erfolgt der automatische Tageswechsel.
            </li>
            <li>
              <IconGraph /> <strong>KPI-Update</strong>: Wirkungen der Entscheidungen werden erst beim Tageswechsel sichtbar.
              Liquidität & G/V in Euro; übrige KPI in Punkten (0–100).
            </li>
            <li>
              <IconDoc /> <strong>Export</strong>: Pro Runde und als Gesamtlauf als PDF exportierbar.
            </li>
          </ul>
        </section>
        ...
        <aside className="ob-start ob-card">
          <header className="ob-start-h">
            <h2>Start</h2>
            <p>Bitte Name eingeben und Rolle(n) bestätigen.</p>
          </header>
          ...
          <label className="ob-field">
            <span>Gewählte Rolle(n)</span>
            <div className="ob-input ro">{roles.join(', ')}</div>
          </label>
          ...
          <div className="ob-start-note">
            Mit Start akzeptieren Sie, dass Entscheidungen protokolliert und in den PDF-Berichten ausgewiesen werden.
          </div>
        </aside>
      </div>
